# Code Review & Development Roadmap (Snapshot from review-and-testing-setup branch)
...
*   **Backend:** The Python code (`embedded_agent`) uses a modular approach (`core` module) but is currently isolated from the frontend. The `actuator.py` shows good attention to detail (humanized mouse movement), but `main.py` is largely a stub.
...
*   **Backend:** Added `pytest`. Created unit tests for `embedded_agent/core/actuator.py` covering both simulation and hardware modes.
...
*   Implement the API/Server in `embedded_agent/main.py`.
...
*   **Backend:** `npm run test:backend` (or `pytest embedded_agent/tests`)
